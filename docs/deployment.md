...
再创建 Actions Variable `SWR_ENABLED=true`，从 Actions 页面手动运行 `Intent Router` 工作流。工作流会关闭 SWR 基础版不支持的 OCI provenance 元数据，发布 `amd64`、`aarch64` 两个架构标签，再生成兼容的 Docker manifest list 作为版本号和 `latest` 标签。
