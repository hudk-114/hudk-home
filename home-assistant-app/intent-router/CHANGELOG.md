...
- 静态 HA 脚本可以声明为 `fallback_when_discovered`；发现同一逻辑能力的真实 HA 实体后，目录、别名解析和执行映射会隐藏该 fallback。
- 霍曼固定一份出粮不再硬编码到“主猫粮机”，避免同一物理喂食器同时显示为静态目标和自动发现目标。
- “给猫加餐”等口语请求直接由能力目录选择唯一喂食器，继续强制二次确认。
